...
uint256 hashGenesisBlock("0x835638aa4511b7482c70a4b91de5e04b8dfee38204779207bdd307eb5728ffd2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kudos: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kudos Signed Message:\n";
...
{
    // Kudos: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 86400 blocks, which will occur approximately every 90 days
    nSubsidy >>= (nHeight / 86400); // Kudos: 86400 blocks in 90 days
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kudos: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kudos: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Kudos: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6458d9895dc9dc9cb15248dbd0ce0145830b0f677471aea1d77bad9deb2008f4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "30.09.2013 BBC - Valve reveals haptic game controller for release in 2014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1381125340;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086273782;

        if (fTestNet)
        {
            block.nTime    = 1380502719;
            block.nNonce   = 387058450;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x05cd9753bf69cc2859cdd4adfd658c05169916e1f49ffced3e98dfea4cb30e0e"));      
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Kudos: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KudosMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KudosMiner : ProcessBlock, block not accepted");
    }
